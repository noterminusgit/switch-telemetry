...
```
switch_telemetry/
├── switch_telemetry_core/      # Domain logic, Ecto schemas, InfluxDB queries
...
- **Collector nodes** -- headless BEAM VMs that maintain gRPC streams and SSH/NETCONF sessions to network devices, writing telemetry to InfluxDB and broadcasting updates via Phoenix.PubSub
- **Web nodes** -- Phoenix LiveView servers that serve modular dashboards, subscribe to real-time PubSub broadcasts, and query InfluxDB for historical data
...
- **InfluxDB** v2 -- time-series metrics storage (open source)
- **Instream** -- InfluxDB v2 Elixir client (Flux queries, line protocol writes)
- **PostgreSQL** 16+ / **Ecto** 3.x -- relational data (devices, dashboards, users, alerts)
...
# - PostgreSQL 16+
# - InfluxDB v2 (for time-series metrics)
